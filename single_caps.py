...
# The first capsule layer will have 32 * 20 * 6 capsules of 8 dimensions each
def make_model(num_gpus, device_index, X, y, caps1_n_maps=32, caps1_n_dims=8):
    ...
    with tf.device(
        ...
        with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
            ...
            # gathers the longest vector's index from the capsule layer
            top_capsule = tf.argmax(
                y_prob_squeezed, axis=2, name="top_capsule")
            y_pred = tf.squeeze(top_capsule, name="y_pred")

            S = tf.one_hot(y_pred, depth=caps2_n_caps, name="S")

            y_pred = tf.cast(y_pred, tf.int64)

            # top_capsule returns what number is predicted in the image
            ...
            present_error = tf.reshape(
                present_error_raw, shape=(-1, 10), name="present_error")
            ...
            absent_error = tf.reshape(
                absent_error_raw, shape=(-1, 10), name="absent_error")
    ...
    with tf.Session(
        ...
        for epoch in range(num_epochs):
            ...
            for iteration in range(n_iterations_per_epoch):
                ...
                print(
                    "\rIteration: {}/{} Loss: {:.5f}".format(
                        iteration, (n_iterations_per_epoch + 1), loss_train),
                    end="")
            ...
            for iteration in range(1, int(n_iterations_validation) + 1):
                ...
                loss_val, acc_val = sess.run(
                    ...
                    feed_dict=feed_dict)
                loss_val = np.mean(loss_val, axis=0)
            ...
            end_epoch = time.time()
            epoch_time = format_time(end_epoch - begin_epoch)

            print(
                "\rEpoch: {} Train accuracy: {:.4f}%  Val accuracy: {:.4f}%  Loss: {:.6f}{}  Duration: {}".
                format(epoch + 1, np.mean(acc_trains), acc_val * 100, loss_val,
                       " (improved)"
